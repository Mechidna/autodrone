...
def get_autonomy_attitude_command(data):
    ...
    # Print only when a new frame arrives
    if not hasattr(get_autonomy_attitude_command, "last_printed_frame_id"):
        get_autonomy_attitude_command.last_printed_frame_id = None

    frame_id = frame["frame_id"]

    if frame_id != get_autonomy_attitude_command.last_printed_frame_id:
        print(
            "controller sees:",
            f"attitude_yaw={attitude['yaw']:.2f}",
            f"imu_accel={imu['accel_xyz']}",
            f"frame_id={frame['frame_id']}",
            f"shape={frame['shape']}",
            f"sim_time_ns={frame['sim_time_ns']}",
            flush=True,
        )

        get_autonomy_attitude_command.last_printed_frame_id = frame_id

    # # Temporary fixed command
    # roll_deg = 0.0
    # pitch_deg = 0.0
    # yaw_deg = 0.0
    # thrust = 0.60
    ...
    if not hasattr(get_autonomy_attitude_command, "last_cmd_print_time"):
        get_autonomy_attitude_command.last_cmd_print_time = 0.0

    if now - get_autonomy_attitude_command.last_cmd_print_time >= 1.0:
        print(
            "autonomy_adapter output:",
        ...
        get_autonomy_attitude_command.last_cmd_print_time = now
...
def update_attitude_angle_flight_control(mavlink_conn, system_boot_ms, data):
    ...
    ROLL_DEG, PITCH_DEG, YAW_DEG, THRUST = get_autonomy_attitude_command(data)
...
class Controller:
    ...
    def update(self):
        ...
        update_attitude_angle_flight_control(self.sim_conn, self.system_boot_ms, self.data)
