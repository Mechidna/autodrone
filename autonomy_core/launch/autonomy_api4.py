...
import time
import math
from autonomy_core.planning.minimum_snap_planner_multi import MultiSegmentMinimumSnapPlanner
...
class AutonomyAPI:
    def __init__(self, use_perception=False):
        ...
        self.gt_gates = [
            ...
            np.array([-0.8, 26.0, 1.5], dtype=float),
            np.array([-0.8, 24.0, 1.5], dtype=float),
            np.array([0.8, 16.0, 1.5], dtype=float),
            np.array([0.0, 8.0, 1.5], dtype=float),
        ]
    ...
    def allocate_segment_times(self, waypoints, current_vel, vmax=2.5, amax=2.0, T_min=1.0):
        """
        Allocate one duration per segment.
        First segment uses actual current velocity.
        Later segments use zero nominal entry velocity for simplicity.
        """
        times = []
    ...
    def compute_final_exit_velocity(self, gates, default_speed=2.5):
        """
        Choose a nonzero terminal velocity so the final segment does not end in a stop.
        """
        if len(gates) >= 2:
            d = gates[-1] - gates[-2]
        else:
            # fallback: nominal forward in +y, consistent with your corrected gate sequence
    ...
    def advance_gate_if_needed(self, threshold=1.25):
        """
        Advance when the drone gets close enough to the current gate.
        """
    ...
    def build_waypoint_horizon(self, current_pos):
        """
        Build a trajectory from current position through all remaining gates.
        """
        remaining_gates = self.gt_gates[self.current_gate_idx:]
        if len(remaining_gates) == 0:
            return np.array([current_pos], dtype=float)

        return np.vstack([current_pos, np.asarray(remaining_gates, dtype=float)])

    def path_plan(self, gate_xyz=None):
        """
        Multi-segment path plan:
        current position -> current gate -> next gate -> ... -> final gate
        """
        ...
        waypoints = self.build_waypoint_horizon(pos)

        if len(waypoints) < 2:
            print("No remaining gates to plan to.")
            return

        times = self.allocate_segment_times(
            waypoints,
            current_vel=vel,
            vmax=2.5,
            amax=2.0,
        ...
        remaining_gates = self.gt_gates[self.current_gate_idx:]
        v_end = self.compute_final_exit_velocity(remaining_gates, default_speed=2.5)

        print("telemetry pos:", pos)
        print("planning waypoint horizon:")
        for i, wp in enumerate(waypoints):
            print(f"  wp[{i}] = {wp}")
        print("segment times:", times)
        print("total horizon:", float(np.sum(times)))
        print("terminal v_end:", v_end)

        self.current_gate_pos = waypoints[1].copy()
        self.active_waypoints = waypoints.copy()
        self.active_times = times.copy()
...
if __name__ == "__main__":
    ...
    # Build first multi-segment trajectory from current pos through all remaining gates
    api.path_plan()
    ...
    while True:
        ...
        # Replan only when:
        # 1) gate index changes, or
        # 2) current trajectory horizon is exhausted
        if gate_changed:
            api.path_plan()
        elif api.time_elapsed >= api.planner.total_time:
            api.path_plan()
